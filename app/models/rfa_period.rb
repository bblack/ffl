class RfaPeriod < ActiveRecord::Base
      ...
      ActiveRecord::Base.transaction do
        self.contracts_eligible.each do |c|
          ...
          if decision.id.nil?
            ...
            decision.team_id = EspnRosterSpot
              .where(espn_player_id: c.player.espn_id)
              .where(roster_revision: self.league.roster_revision)
              .last.team_id
  ...
  def contracts_eligible
    raise StandardError.new("RFA period has been redbuttoned. Check its decisions instead.") if self.redbuttoned
